...
export default function InquiryForm() {
  ...
  function buildWhatsAppText(chosenProducts: string) {
    return [
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const chosenProductNames = initialProducts
      .filter((p) => selectedProducts.includes(p.id))
      .map((p) => `${p.name} (MRP ${p.pricingTiers[0].price})`);
    const chosenProducts = chosenProductNames
      .map((product) => `- ${product}`)
      .join("\n");
    ...
    try {
      const response = await fetch("/api/inquiry", {
        ...
        body: JSON.stringify({ ...form, products: chosenProductNames }),
      });
      ...
      const text = buildWhatsAppText(chosenProducts);
      window.open(
        `https://wa.me/${DEALER_WHATSAPP}?text=${encodeURIComponent(text)}`,
        "_blank"
      );
      setSubmittedViaEmail(false);
      setSubmitted(true);
    } finally {
    ...
    return (
      <div className="text-center py-10">
        ...
        <p className="text-sm text-slate-500 mb-6 max-w-sm mx-auto">
          {submittedViaEmail
            ? "Your inquiry has been sent to info@kagbatteries.in. Our team responds within 2 hours during business hours."
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      >
        ...
        {isSubmitting ? "Sending Inquiry..." : "Send Dealer Inquiry"}
      </button>

      <p className="text-center text-xs text-slate-400">
        This sends the inquiry to info@kagbatteries.in. WhatsApp opens as backup if email is not configured.
      </p>
    </form>
  );
}
